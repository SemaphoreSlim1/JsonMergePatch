...
namespace JsonMergePatch
{
    public class JsonMergePatch<T>
    {
        ...
        private JsonMergePatch(JsonDocument json, JsonSerializerOptions serializerOptions)
        {
            _json = json;
            _serializerOptions = serializerOptions;
        }

        public static JsonMergePatch<T> Create(JsonSerializerOptions serializerOptions = default)
        {
            var doc = JsonDocument.Parse("{ }");
            serializerOptions ??= _defaultSerializerOptions.Value;

            return new JsonMergePatch<T>(doc, serializerOptions);
        }

        public static JsonMergePatch<T> Create(JsonDocument json, JsonSerializerOptions serializerOptions = default)
        {
            serializerOptions ??= _defaultSerializerOptions.Value;

            return new JsonMergePatch<T>(json, serializerOptions);
        }

        public static JsonMergePatch<T> Create(string json, JsonDocumentOptions documentOptions = default, JsonSerializerOptions serializerOptions = default)
        {
        ...
        public bool TryGetObject<TObjProperty>(Expression<Func<T, TObjProperty>> propertyExpr, out JsonMergePatch<TObjProperty> value)
        {
            value = default!;
        ...
        /// <returns>true, if the object was extracted</returns>
        public bool TryGetArray<TElement>(Expression<Func<T, IEnumerable<TElement>>> collectionExpr, out IReadOnlyList<JsonMergePatch<TElement>> value)
        {
            value = default!;
        ...
        public virtual void Set<TProperty>(Expression<Func<T, TProperty>> propertyExpr, TProperty value)
        {
            /*
            var propertyPath = GetPropertyPath(propertyExpr);
